"""
casino_images.py — Pillow-based image generators for AceGold Casino bot.
Blue AceGold brand theme with logo watermark on cards.
"""
...
def _load_logo(size: int = 40) -> Optional[Image.Image]:
    """Load the AceGold logo, resize to square, return RGBA image."""
...
def _draw_card_hidden(draw: ImageDraw.ImageDraw, x: int, y: int):
    """Draw a face-down card with AceGold blue back pattern."""
    x1, y1 = x + CARD_W, y + CARD_H
...
def generate_blackjack_image(
    ...
    """Render a blackjack game state as a branded AceGold PNG."""
    if not _PIL_OK:
        return None
    ...
    try:
        ...
        draw.text((title_x, 14), "AceGold Blackjack", fill=GOLD_ACCENT, font=f_title)
        ...
        bust = p_val > 21
        p_label = f"You  {p_val}" + ("  💥 BUST" if bust else "")
