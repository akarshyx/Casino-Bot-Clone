...
Integration hooks (called from main.py):
    ...
    on_sb_settlement(mid, market, selection)  → SB challenge win tracking
...
def load_events(data: dict):
    ...
    _ev["sb_challenges"]   = src.get("sb_challenges",   {})
    ...
    for ch in _ev["sb_challenges"].values():
        for p in ch.get("participants", {}).values():
            if isinstance(p.get("matches_bet"), list):
                p["matches_bet"] = set(p["matches_bet"])
...
def save_events(data: dict):
    ...
    for ch in snapshot["sb_challenges"].values():
        for p in ch.get("participants", {}).values():
            if isinstance(p.get("matches_bet"), set):
                p["matches_bet"] = list(p["matches_bet"])
...
# ── Sportsbook Challenges ─────────────────────────────────────────────────────
def create_sb_challenge(type_: str, title: str, prize_pool: float,
                        hours: float, requirement: int) -> str:
    cid = _new_id("sbc")
    _ev["sb_challenges"][cid] = {
        "id":           cid,
        "type":         type_,
        "title":        title,
        "prize_pool":   prize_pool,
        "prize_type":   "winner_takes_all" if type_ == "streak" else "top3",
        "start_time":   _now(),
        "end_time":     _now() + hours * 3600,
    ...
    logger.info(f"[EVENTS] SB challenge created: {cid} type={type_} prize=${prize_pool}")
    return cid

def on_sb_settlement(match_id: str, market: str, winning_selection: str):
    now = _now()
    active = [
        (cid, ch) for cid, ch in _ev["sb_challenges"].items()
        if ch.get("active") and now <= ch.get("end_time", 0)
    ]
    if not active:
        return
    try:
        from sportsbook import _sb
        for bid, bet in _sb.get("bets", {}).items():
            if (bet.get("match_id") != match_id
                    or bet.get("market") != market
                    or bet.get("status") != "won"
                    or bet.get("selection") != winning_selection):
                continue
            uid = str(bet["user_id"])
            for cid, ch in active:
                p = ch["participants"].setdefault(uid, {
                    "wins": 0, "streak": 0, "matches_bet": set(), "username": uid,
                })
                p["wins"]   = p.get("wins", 0) + 1
                p["streak"] = p.get("streak", 0) + 1
                p.setdefault("matches_bet", set()).add(match_id)
        # Also reset streak for losers
        for bid, bet in _sb.get("bets", {}).items():
            if (bet.get("match_id") != match_id
                    or bet.get("market") != market
                    or bet.get("status") != "lost"):
                continue
            uid = str(bet["user_id"])
            for cid, ch in active:
                if ch.get("type") == "streak" and uid in ch["participants"]:
                    ch["participants"][uid]["streak"] = 0
    except Exception as e:
        logger.warning(f"[EVENTS] on_sb_settlement error: {e}")

def end_sb_challenge(challenge_id: str) -> list:
    ch = _ev["sb_challenges"].get(challenge_id)
    if not ch:
        return []
    ch["active"] = False
    participants = ch.get("participants", {})
    prize_pool   = ch.get("prize_pool", 0.0)
    ctype        = ch.get("type", "predict")
    req          = ch.get("requirement", 1)

    def score(p):
        if ctype == "streak":    return p.get("streak", 0)
        if ctype == "multimatch": return len(p.get("matches_bet") or [])
        return p.get("wins", 0)

    sorted_p = sorted(participants.items(), key=lambda x: score(x[1]), reverse=True)
    results  = []

    if ch.get("prize_type") == "winner_takes_all":
        if sorted_p:
            uid, p = sorted_p[0]
            s = score(p)
            if s >= req:
                results.append((uid, p.get("username", uid), s, prize_pool))
    else:
        splits = [0.60, 0.30, 0.10]
        for i, (uid, p) in enumerate(sorted_p[:3]):
            s = score(p)
            if s >= req and i < len(splits):
                results.append((uid, p.get("username", uid), s, round(prize_pool * splits[i], 2)))

    logger.info(f"[EVENTS] SB challenge {challenge_id} ended: {len(results)} winners")
    return results
...
def build_events_panel() -> tuple:
    ...
    active_sbc = [c for c in _ev["sb_challenges"].values()
                  if c.get("active") and now <= c.get("end_time", 0)]
    race       = _ev.get("deposit_race", {})
    ...
    lines.append(f"\n⚽ SB Challenges: <b>{len(active_sbc)} active</b>")
    for c in active_sbc[:2]:
        lines.append(f"  • {c['title']} ({c['type']}) ⏳{_fmt_time_left(c['end_time'])}")
    ...
    kb = [
        ...
        [InlineKeyboardButton("💸 Cashback",        callback_data="ev_cashback"),
         InlineKeyboardButton("⚽ SB Challenges",   callback_data="ev_sbchallenge")],
...
def build_sbchallenge_panel() -> tuple:
    challenges = _ev["sb_challenges"]
    now        = _now()
    lines      = ["⚽ <b>SPORTSBOOK CHALLENGES</b>\n━━━━━━━━━━━━━━━━━━━━━━"]
    btns       = []

    for cid, ch in list(challenges.items()):
        expired = not ch.get("active") or now > ch.get("end_time", 0)
        status  = "❌ Ended" if expired else "✅ Active"
        lines.append(
            f"\n<b>{ch['title']}</b> [{status}]\n"
            f"  Type: <b>{ch['type']}</b> | Need {ch['requirement']} | "
            f"Prize: ${ch.get('prize_pool',0):.0f}\n"
            f"  Participants: {len(ch.get('participants',{}))}"
        )
        if not expired:
            lines[-1] += f" | ⏳{_fmt_time_left(ch['end_time'])}"
            btns.append([
                InlineKeyboardButton(f"🏁 End ({ch['title'][:12]})", callback_data=f"ev_sbc_stop_{cid}"),
                InlineKeyboardButton("🗑", callback_data=f"ev_sbc_del_{cid}"),
            ])
        else:
            btns.append([InlineKeyboardButton("🗑 Remove", callback_data=f"ev_sbc_del_{cid}")])

    if not challenges:
        lines.append("\nNo SB challenges yet.")

    lines.append(
        "\n━━━━━━━━━━━━━━━━━━━━━━\n"
        "<b>Create:</b>\n"
        "<code>/sbchallenge streak Prediction King 500 48 5</code>\n"
        "→ Win 5 consecutive bets → takes $500\n\n"
        "<code>/sbchallenge predict Top Predictor 300 72 10</code>\n"
        "→ Most correct predictions in 72h (top-3 split $300)\n\n"
        "<code>/sbchallenge multimatch Bet Explorer 200 24 5</code>\n"
        "→ Bet on 5 different matches within 24h"
    )
    btns.append([InlineKeyboardButton("🔄 Refresh", callback_data="ev_sbchallenge"),
                 InlineKeyboardButton("« Back",     callback_data="ev_panel")])
    return "\n".join(lines), InlineKeyboardMarkup(btns)
...
def build_help_panel() -> tuple:
    text = (
        ...
        "<b>⚽ SB Challenges</b>\n"
        "<code>/sbchallenge streak TITLE PRIZE HOURS REQ</code>\n"
        "<code>/sbchallenge predict TITLE PRIZE HOURS REQ</code>\n"
        "<code>/sbchallenge multimatch TITLE PRIZE HOURS REQ</code>\n\n"
...
async def handle_ev_callback(query, context, uid: str, data: str, cb: dict):
    ...
    # ── SB challenge sub-panel ────────────────────────────────────────────────
    if data == "ev_sbchallenge":
        await answer()
        text, kb = build_sbchallenge_panel()
        await edit(text, kb)
        return

    if data.startswith("ev_sbc_stop_"):
        cid     = data.replace("ev_sbc_stop_", "")
        results = end_sb_challenge(cid)
        cb["save"]()
        medals = ["🥇", "🥈", "🥉"]
        if not results:
            await answer("⚽ Challenge ended — no qualifying winners.", alert=True)
        else:
            for i, (uid_w, name, score, prize) in enumerate(results):
                cb["add_balance"](uid_w, prize, "sb_challenge_prize")
                cb["deduct_house"](prize)
                if cb.get("log_tx"):
                    cb["log_tx"](uid_w, prize, "sb_challenge_prize", f"SB Challenge score={score}")
                try:
                    await cb["send_dm"](uid_w,
                        f"⚽ <b>SB Challenge Won!</b>\n"
                        f"Score: <b>{score}</b> | Prize: <b>${prize:.2f}</b> credited!"
                    )
                except Exception:
                    pass
            cb["save"]()
    ...
    if data.startswith("ev_sbc_del_"):
        cid = data.replace("ev_sbc_del_", "")
        ch  = _ev["sb_challenges"].pop(cid, None)
        cb["save"]()
        if ch:
            await answer(f"🗑 Removed: {ch['title']}", alert=True)
        text, kb = build_sbchallenge_panel()
        await edit(text, kb)
        return
